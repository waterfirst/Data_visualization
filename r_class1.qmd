...
-   \[심화\] penquin 데이터를 표준화(평균 0, 표준편차 1), 정규화(1과 0 사이로 만듦) 하라

-   \[심화\] airquality 데이터셋에서 Ozone 값 중 이상값을 제외한 평균을 구하라(이상값은 평균에서 1.5\*표준편차 이상값)
...
### [3. 강수량 분석]{style="color:blue;"}

#### 2020년도 이화동(서울) , 수영동(부산)의 시간단위의 기온과 강수량 분석\] {style="color:blue;"}
